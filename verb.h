...
class Verb{
  ...
  private:
    ...
    struct ConjRules{
      ...
      struct ImperfectTense{
        std::string ar[5];
        std::string erirIr[5];
      };
      ImperfectTense imperfect;
    };
    ...
    static std::string imperfectIrregularVerbs[3];//the list of verbs that are irregular in the imperfect tense
    ...
    std::string presentStem;// same as stem, only included if nesecary, included for easier modification if I missed something
    ...
    //IMPERFECT
    bool isImperfectRegular;
    std::string imperfectConjugations[5];
    ...
    //IMPERFECT
    void setIsImperfectRegular();//checks if the verb is ir, ser, or ver
    void setImperfectConjugations();//sets the conjugations for the imperfect tense.
    ...
    void setImperfectTense();//sets all things for the imperfect tense
